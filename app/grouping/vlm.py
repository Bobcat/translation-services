...
_USER_INSTRUCTION = (
    ...
    "2. **Importance:** Output a t for title, h for header, b for body or a m for "
    "metadata/footers.\n\n"
    "3. **Reading order:** Process the text from top to bottom, following the natural "
    "reading order. For every piece of text, immediately precede it with its "
    "classification.\n\n"
    "4. **Icons:** Ignore graphical icons and pictograms. They are not text and not part "
    "of any element; do not output them or a name/placeholder for them.\n\n"
    "5. **Table rows:** If an element is a **table row**, put '|' between its fields.\n\n"
    "6. **Field values:** If an element has the format <field-label> <field-value>, put "
    "'|' between the label and the value.\n\n"
    "7. **Bullet-list items:** If an element has the format <bullet> <item text>, output the "
    "element as |@blt|<bullet>|<item text>.\n\n"
    "8. **Font-family:** For *font-family* provide your best best guess for a **specific "
    "font name** do NOT just mention serif or sans-serif.\n\n"
    "9. **Alignment:** Determine the text elements position on the document. Put an l for "
    "left, c for centered or an r for right inside the label. Table rows are never "
    "centered.\n\n"
    "# OUTPUT FORMAT (EXACT)\n"
