...
| `POST` | `/v1/requests/{id}/rerender` | Re-render a completed request's cached translations with new render flags (`render_size_mode`/`erase_fill_mode`/`width_fit_mode`/`size_metric_mode`/`size_cohort_mode`); no new translation. An image source re-renders its image; a document source re-renders every page from its own cached translations and reassembles the PDF. |
